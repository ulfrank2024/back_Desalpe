...
const deactivateExpiredLinks = async () => {
    ...
    try {
        ...
        const { data: expiredLinks, error: fetchError } = await supabase
            ...
            .select('id, code_court, ambassadeur_prenom, ambassadeur_email, nombre_clics, date_creation, valide_jusqu_a')
            ...
            try {
                ...
                try {
                    ...
                    if (link.ambassadeur_email) {
                        await emailService.sendEmail(link.ambassadeur_email, subject, null, fullHtmlContent);
